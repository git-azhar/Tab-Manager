...
//  STATE
// ══════════════════════════════════════════════════════
let S = null            // full state from background
    ...
    function attempt(retries) {
      chrome.runtime.sendMessage(msg, r => {
        if (chrome.runtime.lastError) {
          // SW may be sleeping — retry up to 3x
...
document.getElementById('btn-sweep').addEventListener('click', async () => {
  await send('SWEEP_NOW')
  toast('⚡ Sweep triggered')
  await refresh()
})
...
function renderBanner(s) {
  ...
  document.getElementById('ss-sweep').textContent   = settings.sweepMinutes < 1
    ? Math.round(settings.sweepMinutes * 60) + 's'
    : settings.sweepMinutes + 'm'
...
function renderTabs(tabInfos) {
  ...
  const allCount = tabInfos ? tabInfos.length : 0
  if (!sorted.length) {
    list.innerHTML = `<div class="empty-state"><div class="ei">🌐</div><div class="et">No tabs visible</div><div class="es">${allCount} tab(s) in total. Chrome internal tabs are hidden.</div></div>`
    return
  }
  ...
  list.querySelectorAll('[data-act]').forEach(btn => {
    btn.addEventListener('click', async e => {
      ...
      if (act === 'close') {
        await send('CLOSE_TAB', {}, { tabId: id })
        toast('✕ Tab closed')
        await refresh()
      } else if (act === 'reset') {
        await send('RESET_TAB', {}, { tabId: id })
        toast('🔄 Timer reset')
        await refresh()
      } else if (act === 'focus') {
...
function renderRules(rules) {
  ...
  list.querySelectorAll('.rule-toggle').forEach(tog => {
    tog.addEventListener('change', async () => {
      ...
      toast(tog.checked ? '✅ Rule enabled' : '⏸ Rule disabled')
      await refresh()
    })
  })
...
bindSlider('s-global',    'sv-global', 'm')
bindSlider('s-sweep-set', 'sv-sweep',  'm')
bindSlider('s-warn',      'sv-warn',   'm')
bindSlider('s-idle',      'sv-idle',   's')
...
function renderAnalytics(a) {
  ...
  list.innerHTML = a.history.slice(0, 15).map(h => {
...
// Wire favicon error fallback (avoids CSP-violating onerror attribute)
...
async function refresh() {
  let resp
  try {
    resp = await send('GET_FULL_STATE')
  } catch(e) { return }
  
  if (!resp || !resp.settings) {
    // SW not ready yet — show partial state
    const list = document.getElementById('tab-list')
    if (list && list.querySelector('.et') && list.querySelector('.et').textContent === 'Connecting…') {
      // still loading, will retry
    }
    return
  }
  S = resp
  ...
  // Only render analytics if on that panel
...
async function loadMemory() {
  const data = await send('GET_MEMORY')
  ...
  const maxTab     = withMem.sort((a, b) => b.mem.used - a.mem.used)[0]
  ...
  // Sort: matched first, then by memory desc
  const sorted = [...tabs].sort((a, b) => {
    if (a.matched && !b.matched) return -1
    if (!a.matched && b.matched) return 1
    const am = a.mem?.used || 0, bm = b.mem?.used || 0
    return bm - am
  })
  ...
  sorted.forEach(tab => {
    ...
    if (hist.length >= 3) {
      ...
      if (delta > 2 * 1024 * 1024) {
        growthHtml = `<span class="mem-growth up">↑ growing</span>`
      } else if (delta < -512 * 1024) {
        growthHtml = `<span class="mem-growth stable">↓ freeing</span>`
      } else {
        growthHtml = `<span class="mem-growth stable">● stable</span>`
      }
    }

    const el = document.createElement('div')
    el.className = 'mem-card' + (tab.matched ? '' : ' unmatched')

    const favHtml = S?.tabInfos?.find(t => t.id === tab.tabId)?.favIconUrl
      ? `<img class="fav-img" src="${escHtml(S.tabInfos.find(t => t.id === tab.tabId).favIconUrl)}"/>`
      : '🌐'
